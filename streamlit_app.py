...
def load_and_prepare(uploaded):
    ...
    for req in ['Категория','Группа','Name_tov','Формат','Склад']:
        if req not in df.columns:
            raise KeyError(f"Нет колонки «{req}»")

    # Продажи
    sale_col = next((c for c in df.columns if 'продажа' in c.lower() and 'сумма' in c.lower()), None)
    if not sale_col: raise KeyError("Нет колонки с суммой продаж")
    ...
    if not fill_col: raise KeyError("Нет колонки закрытия")
    ...
    if not (col_zc2 and col_srok and col_kach):
        raise KeyError("Нет колонок списания ZC2/срок/качество")
    ...
    waste = grp.apply(lambda x: np.average(x['Списания %'],
                                           weights=x['Продажа с ЗЦ сумма'])
                       if x['Продажа с ЗЦ сумма'].sum()>0 else x['Списания %'].mean())
    fill = grp.apply(lambda x: np.average(x['Закрытие потребности %'],
                                          weights=x['Продажа с ЗЦ сумма'])
    ...
    wmap = agg.groupby('Группа').apply(
        lambda x: np.average(x['Списания %'],
                             weights=x['Продажа с ЗЦ сумма'])
        if x['Продажа с ЗЦ сумма'].sum()>0 else x['Списания %'].mean())
    agg['group_weighted_mean_waste'] = agg['Группа'].map(wmap.to_dict())
...
def display_anomaly_table(df,title):
    ...
    rename={'group_mean_waste':'Среднее в группе %',
    ...
    styler=(df[cols].rename(columns=rename)
            .style.format({c:'{:.1f}' for c in ['Списания %','Среднее в группе %','Средневзв. в группе %','Закрытие потребности %']}
                        |{ 'Продажа с ЗЦ сумма':'{:.0f}', 'Степень аномалии':'{:.3f}','Скор в группе':'{:.3f}'})
...
def main():
    ...
    if sel_fmts: df=df[df['Формат'].isin(sel_fmts)]
    if sel_whs: df=df[df['Склад'].isin(sel_whs)]
    if df.empty:
        st.warning("Нет данных после фильтров")
        return

    # Выручка: слайдер + ручной ввод

...
def load_and_prepare(uploaded):
    ...
    # --- 3. Убираем некорректные строки ---
    df = df.dropna(subset=['Группа','Name_tov','Списания %','Закрытие потребности %'])

    # --- 4. Агрегируем по SKU (категория–группа–товар) ---
    def agg_group(g):
    ...
    # --- 5. Внутригрупповые метрики списаний ---
...
def score_anomalies(df):
    ...
    # рассчитываем IsolationForest внутри каждой группы
...
def main():
    ...
    sale_range = st.sidebar.slider(
        "Выручка (₽)", sale_min, sale_max, sale_def, step=1
    )
    sale_min_in = st.sidebar.number_input(
        "Мин. выручка", sale_min, sale_max, value=sale_range[0]
    )
    sale_max_in = st.sidebar.number_input(
        "Макс. выручка", sale_min, sale_max, value=sale_range[1]
    )
